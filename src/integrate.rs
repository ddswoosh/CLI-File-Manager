...
pub fn read(hashmap: &mut HashMap<String, String>) -> Result<String, String> {
    ...
    match command {
        // "nd" => controls::Ndir::new_directory(name.to_string(), path),
        // "dd" => controls::Ddir::delete_directory(name.to_string(), path),
        // "odd" => controls::Ddir::override_delete(name.to_string(), path),
        // "nf" => controls::Nfil::new_file(name.to_string(), ext.to_string(), hashmap),
        // "df" => controls::Dfil::delete_file(name.to_string(), path),
        "mov" => controls::Mov::mov(name.to_string(), ext.to_string(), path),
        &_ => todo!()
    }?;
