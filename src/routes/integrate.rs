...
pub fn read(hashmap: &mut HashMap<String, String>, cur_holding: &mut [Option<PathBuf>; 1], editors: &mut HashMap<String, String>, cur_command: &mut String) -> String {
    ...
    *cur_command = cin.clone();
    ...
    if &vec[0] == &"-help" {
        ...
        if vec.len() == 3 {
            ...
            match command {
                ...
                "mov" => {
                    if name == "holding".to_string(){
                        ...
                        return controls::Mov::mov(name.to_string(), ext.to_string()).unwrap()
                },
            ...
            match command {
                "nd" => return controls::Dir::new_directory(name.to_string(), path).unwrap(),
                "dd" => return controls::Dir::delete_directory(name.to_string(), path).unwrap(),
                "odd" => return controls::Dir::override_delete(name.to_string(), path).unwrap(),
                "df" => return controls::Fil::delete_file(name.to_string(), path).unwrap(),
                "grab" => return controls::FileArray::grab(name.into(), cur_holding).unwrap(),
                "cd" => return controls::Environment::change_dir(name.to_string()).unwrap(),
                _ => return "Command not accepted, please type -help to see the list of all commands.".to_string()
            }
